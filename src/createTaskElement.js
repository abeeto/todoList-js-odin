...
export default function createTaskElement({ taskObj, projectName }) {
  const taskTitleInfoNode = ElementsHelper.createGenericElement({
    elementTagName: "div",
    classList: ["flex", "flex-item-grow", "taskHeadingText"],
    innerText: taskObj.getName(),
    attributesMap: { "data-is-done": taskObj.getIsDone() },
  });
  const taskDescInfoNode = ElementsHelper.createGenericElement({
    elementTagName: "div",
    classList: [
      "flex",
      "flex-item-grow",
    ...
    innerText: taskObj.getDescription(),
  });
  const taskPriorityNode = ElementsHelper.createGenericElement({
    elementTagName: "div",
    classList: ["flex", "taskBaseText"],
    innerText: "Priority: " + taskObj.getPriority().toUpperCase(),
  });
  const taskDueDateNode = ElementsHelper.createGenericElement({
    elementTagName: "div",
    classList: ["flex", "taskBaseText"],
    innerText: "Due Date: " + taskObj.getDueDate(),
    attributesMap: { "data-is-done": taskObj.getIsDone() },
  });
  ...
  const expandButtonNode = ElementsHelper.createGenericElement({
    ...
    innerText: "Expand",
    clickEventCallBack: function () {
      hideableNodes.forEach(
        (node) =>
          (node.dataset.isHidden =
            node.dataset.isHidden === "true" ? "false" : "true"),
      );
      this.innerText =
        taskDescInfoNode.dataset.isHidden === "true" ? "Expand" : "Hide";
    },
  });
