...
export default function createEditableTaskView({ taskObj, projectName }) {
  ...
  taskInfoSetters = taskInfoSetters.map((taskInfoSetter) =>
    taskInfoSetter.bind(taskObj),
  );
  const taskInfoItemsFormObjectNames = [
    "name",
    "dueDate",
    "priority",
    "description",
  ];
  const taskInfoItemsIds = [
    "task-editable-name",
    "task-editable-due-date",
    "task-editable-priority",
  ...
  const taskInfoItemClassList = [
    ["taskEditInput", "taskHeadingText"],
    ["taskEditInput", "taskBaseText", "max-content-width"],
    ["taskEditInput", "taskBaseText", "max-content-width"],
    ["taskEditInput", "taskDescription", "taskBaseText", "flow-y-bottom"],
  ];
  ...
  for (let i = 0; i < taskInfoItems.length; i++) {
    const inputItem = ElementsHelper.createGenericElement({
      ...
      attributesMap: {
        value: taskInfoItems[i],
        id: taskInfoItemsIds[i],
        type: taskInfoItemsInputType[i],
        name: taskInfoItemsFormObjectNames[i],
      },
